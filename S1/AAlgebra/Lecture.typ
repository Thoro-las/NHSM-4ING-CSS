...
#ooc[
  *Constructive Proof:* Let $q = card E$ and $q - 1 = product_(i=1)^k p_i^(l_i)$ by the fundamental theorem of arithmetic. Consider for $i in [|1, k|]$ the polynomials $P_i (X) = X^((q-1)/p_i) - 1$, notice that $card cal(Z)(P_i) <= (q-1)/q_i < q-1 = card E^*$, thus there exists $y_i in E^* \\cal(Z)(P_i)$. Now for $i in [|1, k|]$ $z_i = y_i^((q-1)\/(p_i^(l_i)))$, notice that $z_i$ has order $p_i^(l_i)$ since $z_i^(p_i^(l_i))=y_i^(q-1)=1$ by Lagrange's theorem and $z_i^(p_i^(l_i - 1)) = y_i^((q-1)\/p_i) != 1$ by the definition of $y_i$. We chose $z = product_(i=1)^n z_i$, since all the orders of $z_i$ are coprime from the fact that they are different primes, then the order of $z$ is $product_(i=1)^n p_i^(l_i) = q - 1 = card E^*$, thus $E^* = chevron.l z chevron.r $ and $E^*$ is cyclic.
  ...
  #exm[
    ...
    $
      ...
      => z = 6 dot 2 = 5
    $
    It is easy to verify that $chevron.l 5 chevron.r = FF_7^star$.
  ]
]
...
#prf[
  Consider $theta$ the generator of $E^*$ from the previous proposition. $F(theta) subset E$ since $theta in E^* subset E$ and $F subset E$. $E subset F(theta)$ given that $E^* = chevron.l theta chevron.r subset F(theta)$ and $0 in F subset F(theta)$ thus we obtain $E = {0} union E^* subset F(theta)$ hence $E = F(theta)$.
]
...
#prf[
  ...
  There $n$ embeddings $f^1, dots, f^n$, now we will prove that they are all not equal, that is $forall 1 <= i < j <= n, f^i equiv.not f^j$. Suppose that there exists $i, j in [|1, n|]$, $i < j$ such that $f^i equiv f^j$, $FF_p (alpha)$ is a finite field then there is a generator $theta$ for the multiplicative group $(FF_p (alpha))^* = chevron.l theta chevron.r$, $theta^(p^i) = f^i (theta) = f^j (theta) = theta^(p^j)$, so $theta^(p^j - p^i) = 1$ but $p^j - p^i < p^n$ and the order of $theta$ is $p^n$ which is a contradiction. Thus, we conclude that ${f^i}_(i in [|1, n|])$ are the only embeddings of $FF_p (alpha)$ into $overline(FF_p)$.
]
...
#def(name: "Cyclic", count: false)[
  Let $G$ be a group and $g in G$, we say that $G$ is cyclic $G = chevron.l g chevron.r = {g^n | n in NN}$.
]
...
// #prf[
//   Recall that if $card G = p$ prime by Lagrange there are no trivial subgroups in $G$. Since $G != {e}$ then $exists g in G\\{e}$, then $G = chevron.l g chevron.r$ so $G$ is cyclic.
// ]
